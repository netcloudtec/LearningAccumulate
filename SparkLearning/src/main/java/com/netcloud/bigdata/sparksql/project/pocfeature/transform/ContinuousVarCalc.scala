...
object ContinuousVarCalc{
  ...
  def continuousTrainVarCalc(inputDf: Dataset[Row], inputCalcDf: Dataset[Row], mccFile: String, outputFile: String, saveMode: String, dropcolumn: String, discretecolumns: Array[String], discreteDefault: String) = {
    ...
    val dfThreeMon = dfInsertMccBigClassColumn.na.fill(0.0, Seq("MD_TRAN_AMT3"))
    ...
    val dfBasic = dfThreeMon.select(Seq(col("*")).++(basicAmtColumn.drop(1)): _*)
    ...
    val dfTimes = dfBasic.withColumn("LOCAL_UNIX_TIMESTAMP", unix_timestamp(
                         ...
                         .withColumn("timesId", concat_ws("_", col("SD_PAN"), col("DD_DATE"), col("SD_TRACE_NBR")))
                         .drop("DD_DATE, LOCAL_DATE, LOCAL_TIME, LOCAL_UNIX_TIMESTAMP, MD_TRAN_AMT1, MD_TRAN_AMT3, SD_PAN, SD_RETL_SIC_CDE, SD_RETL_ID, SD_TRAN_CDE_CAT, SD_ORIG_CRNCY_CDE, SD_ORIG_WIR_CNTRY_CDE, SD_CITYNO, SD_FRWD_INST_ID_NUM, SD_TRACE_NBR".split(", "):_*)

    println("dfTimes columns:" + dfTimes.columns.mkString(", "))
    val dfSample = inputDf.withColumn("timeId", concat_ws("_", col("SD_PAN"), unix_timestamp(date_format(col("DD_DATE"), "yyyy-MM-dd HH"))))
                          .withColumn("timesId", concat_ws("_", col("SD_PAN"), col("DD_DATE"), col("SD_TRACE_NBR")))

    println("dfSample columns:" + dfSample.columns.mkString(", "))
    println("signal 8")
    ...
    val lastcolumns = dfSample.columns.diff(dfTime.columns).++(dfTime.columns).diff(dfTimes.columns).++(dfTimes.columns).filterNot(dropcolumns.contains(_))

    println("lastcolumns:" + lastcolumns.mkString(", "))
    val dfTemp = dfSample.join(dfTime, dfSample.col("timeId") === dfTime.col("timeId"), "left")
    println("dfTemp columns:" + dfTemp.columns.mkString(", "))
    dfTemp.join(dfTimes, dfTemp.col("timesId") === dfTimes.col("timesId"), "left")
  ...
  def calcTrainSetVar(prop: Properties, spark: SparkSession){
    ...
    val inputCalcFile = prop.getProperty("train_sample_calc")
    val mccFile = prop.getProperty("mcc_file")
    val saveMode = prop.getProperty("saveMode")
    val samplenum = prop.getProperty("samplenum").toInt
    ...
    for(i <- 0 to samplenum - 1){
      ...
      val inputCalcDf = spark.read.option("mergeSchema", true)
                                    .parquet(inputCalcFile + "/" + i)
                                    .withColumn("MD_TRAN_AMT3", col("MD_TRAN_AMT3").cast(DoubleType))

      val outputFile = inputFile + "/" + i + "_var_output"
      continuousTrainVarCalc(inputDf, inputCalcDf, mccFile, outputFile, saveMode, dropcolumn, discretecolumns, discreteDefault)
    }
  }
  ...
  */
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = SparkSession.builder().appName(s"spark_calc_var_${funcName}")
